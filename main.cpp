    ...
    while (window.isOpen() && R==0)
    {
        ...
        else if(arrows.size()!=0)
        {
            for(int i=0;i<arrows.size();i++)
            {
                ...
                {
                    if(main_p.getX()>=arrows[i]->getShape()->getPosition().x && main_p.getX()<=arrows[i]->getShape()->getPosition().x+arrows[i]->getSize_x() && main_p.getY()-main_p.getShape().getOrigin().y<=arrows[i]->getShape()->getPosition().y+arrows[i]->getSize_y() && main_p.getY()+main_p.getShape().getOrigin().y>=arrows[i]->getShape()->getPosition().y)
                        main_p.health_d--;
        ...
        for(int i=0;i<enemies.size();i++)
        {
            ...
            else
            {
                ...
                if(enemies[i]->m_health!=0 && (sf::Keyboard::isKeyPressed(sf::Keyboard::E)) && main_p.getX()+main_p.getShape().getOrigin().x>=(enemies[i]->getX()-enemies[i]->getShape()->getOrigin().x) && main_p.getX()-main_p.getShape().getOrigin().x<=(enemies[i]->getX()+enemies[i]->getShape()->getOrigin().x) && number%50==0 && -main_p.getShape().getOrigin().y<=(main_p.getShape().getPosition().y-enemies[i]->getShape()->getPosition().y) && (main_p.getShape().getPosition().y-enemies[i]->getShape()->getPosition().y)<=main_p.getShape().getOrigin().y)
                    enemies[i]->m_health-=1;
                if(main_p.health_d>0 && main_p.getX()>=(enemies[i]->getX()-enemies[i]->getShape()->getOrigin().x) && main_p.getX()<=(enemies[i]->getX()+enemies[i]->getShape()->getOrigin().x) && number%50==0 && -main_p.getShape().getOrigin().y<=(main_p.getShape().getPosition().y-enemies[i]->getShape()->getPosition().y) && (main_p.getShape().getPosition().y-enemies[i]->getShape()->getPosition().y)<=main_p.getShape().getOrigin().y)
        ...
        else if(main_p.getX()>=59*brick.sizeX()+brick.sizeX()/2 && timer_final>=1000)
        {
            ...
            if(pad<=height*2-30) { main_p.getShape().move(0,1); view.move(0,1);}
            pad++;
        ...
        for(int i=0;i<main_p.health_d;i++)
            window.draw(main_p.getHealth()[i]);
        window.display();
        number++; t_a++;
    }
    return 0;
